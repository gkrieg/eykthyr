...
class Eykthyr():
    ...
    def compute_metagenes(self,
        ...
        if self.rna_preprocessed == False:
            print('RNA appears to not be preprocessed. Please preprocess RNA using Eykthyr.preprocess_rna() or set Eykthyr.rna_preprocessed = True')
            return
    ...
    def analyze_metagenes(self,
        ...
        tl.leiden(self.popari, use_rep="normalized_X", target_clusters=num_leiden_clusters)
    ...
    def compute_TF_activity(self,
        ...
        tfpeaks.rename(columns={c:new_c for c, new_c in zip(tfpeaks.columns, new_col_names)}, inplace=True)
        tfpeaks = tfpeaks.T
    ...
    def run_all_perturbations(self,
        ...
        if not self.TF:
            print('TF activity needs to be computed by ArchR. Please follow the jupyter notebook for\n'
                  'preprocessing ATAC-seq data and then run compute_TF_activity().')
            return

        if not self.edge_weights:
            print('GRN edge weights need to be computed first, please run compute_TF_metagene_weights().')
            return
